...
export const fetchPokemonDetail = async (name: string):Promise<PokemonDetailType> => {
  ...
  const response = await remote.get<PokemonDetailReponseType>(PokemonDefaultUrl)
  const speciesResponse = await remote.get<PokemonSpeciesReponseType>(PokemonSpeciesUrl)
